...
from data_modules import (
    TOFU_TextDatasetQA, TOFU_data_collator_with_indices,
    KnowUnDo_TextDatasetQA, KnowUnDo_data_collator_with_indices
)
from data_modules import get_batch_loss
...
def eval_perturbation_ratio(eval_dataloader, perturb_dataloader, model):
    ...
    for batch, perturb_batch in tqdm(zip(eval_dataloader, perturb_dataloader)):
        ...
        try:
            ...
            perturb_loss_per_token = dict(zip(indices.cpu().float().tolist(), perturb_loss_per_token.cpu().float().tolist()))
            gt_loss_per_token = dict(zip(indices.cpu().float().tolist(), gt_loss_per_token.cpu().float().tolist()))
            truth_ratio = dict(zip(indices.cpu().float().tolist(), truth_ratio.cpu().float().tolist()))
            gt_loss = dict(zip(indices.cpu().float().tolist(), gt_loss.cpu().float().tolist()))
            perturb_loss = dict(zip(indices.cpu().float().tolist(), perturb_loss.cpu().float().tolist()))
            num_token_gt = dict(zip(indices.cpu().float().tolist(), num_token_gt.cpu().float().tolist()))
            num_token_perturb = dict(zip(indices.cpu().float().tolist(), num_token_perturb.cpu().float().tolist()))
...
def get_dataloader(cfg, eval_task, tokenizer, folder, split, question_key, answer_key, base_answer_key, perturbed_answer_key):
    ...
    base_torch_format_dataset = TOFU_TextDatasetQA(
        folder,
        tokenizer=tokenizer,
        model_family=cfg.model_family,
        max_length=cfg.generation.max_length,
    ...
    perturb_torch_format_dataset = TOFU_TextDatasetQA(
        folder,
        tokenizer=tokenizer,
        model_family=cfg.model_family,
        max_length=cfg.generation.max_length,
    ...
    eval_dataloader = torch.utils.data.DataLoader(
        torch_format_dataset, batch_size=cfg.batch_size, collate_fn=TOFU_data_collator_with_indices
    )
    base_eval_dataloader = torch.utils.data.DataLoader(
        base_torch_format_dataset, batch_size=max(1, cfg.batch_size//4), collate_fn=TOFU_data_collator_with_indices
    )
    perturb_dataloader = torch.utils.data.DataLoader(
        perturb_torch_format_dataset, batch_size=max(1, cfg.batch_size//4), collate_fn=TOFU_data_collator_with_indices
    )

    return eval_dataloader, base_eval_dataloader, perturb_dataloader

def get_single_dataloader(cfg, eval_task, tokenizer, folder, split, question_key, answer_key, batch_size):
    if "tofu" in folder.lower():
        torch_format_dataset = TOFU_TextDatasetQA(
            folder,
            tokenizer=tokenizer,
            model_family=cfg.model_family,
        ...
        data_collator = TOFU_data_collator_with_indices
    elif "knowundo" in folder.lower():
        torch_format_dataset = KnowUnDo_TextDatasetQA(
            folder,
            tokenizer=tokenizer,
            model_family=cfg.model_family,
            max_length=cfg.generation.max_length,
            ...
            question_key=question_key,
            answer_key=answer_key,
            data_type=(cfg.data_name).split('-')[1]
        )
        data_collator = KnowUnDo_data_collator_with_indices

    if cfg.ds_size:
        torch_format_dataset.data = torch_format_dataset.data.select(range(min(cfg.ds_size, len(torch_format_dataset.data))))

    eval_dataloader = torch.utils.data.DataLoader(
        torch_format_dataset, batch_size=batch_size, collate_fn=data_collator
    )
...
def get_all_evals(cfg, model, tokenizer, eval_task, eval_dataloader, base_eval_dataloader, perturb_dataloader, normalize_gt=False):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        try:
            ...
            eval_logs['avg_gt_loss'].update(dict(zip(indices.cpu().float().tolist(), gt_loss_per_token.cpu().float().tolist())))
            eval_logs['gt_loss'].update(dict(zip(indices.cpu().float().tolist(), gt_loss.cpu().float().tolist())))
            eval_logs['num_token_gt'].update(dict(zip(indices.cpu().float().tolist(), num_token_gt.cpu().float().tolist())))
            eval_logs['generated_text'].update(dict(zip(indices.cpu().float().tolist(), zip(input_string, gen_output,gt))))

    eval_logs.update(eval_rouge_recall(gen_outputs, ground_truths, all_indices))

    if base_eval_dataloader and perturb_dataloader:
...
def main(cfg):
    ...
    for i, (folder, split, question_key, answer_key, eval_task, base_answer_key, perturbed_answer_key) in enumerate(zip(cfg.data_path, cfg.split_list, cfg.question_key, cfg.answer_key, cfg.eval_task, cfg.base_answer_key, cfg.perturbed_answer_key)):
        ...
        eval_dataloader = get_single_dataloader(cfg, eval_task, tokenizer, folder, split, question_key, answer_key, cfg.batch_size)
        base_eval_dataloader, perturb_dataloader = None, None
        if "tofu" in folder.lower():
            base_eval_dataloader = get_single_dataloader(cfg, eval_task, tokenizer, folder, split, question_key, base_answer_key, max(1, cfg.batch_size//4))
            perturb_dataloader = get_single_dataloader(cfg, eval_task, tokenizer, folder, split, question_key, perturbed_answer_key, max(1, cfg.batch_size//4))
