...
from utils import get_datapoint_hash, merge_dicts, interleave_eval_result_dict, get_forget_quality, get_model_utility

# def printll(name, inp):
#     #print list with 4 decimal for each item
#     print(name, [round(x, 4) for x in inp])

def freeze_params_hook(grad):
    # raise RuntimeError("Trying to compute gradient for frozen parameter") # for debugging
    return None
...
class CustomTrainerForgetting(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.influence_param_map = kwargs.pop('influence_param_map')
        self.hooks = []
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if len(self.influence_param_map) > 0:
            datapoint_hash = get_datapoint_hash(inputs[0][1]) # inputs = (forget_inputs, retain_inputs)
            localized_params = self.influence_param_map[datapoint_hash]
            for hook in self.hooks:
                hook.remove()
            for param_name, param in model.named_parameters():
                if param_name not in localized_params:
                    self.hooks.append(param.register_hook(freeze_params_hook))
                # if param_name in localized_params:
                #     param.requires_grad = True
                # else:
                #     param.requires_grad = False

        if self.loss_type == "grad-ascent":
