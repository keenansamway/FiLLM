"""
From 'Token-wise Influential Training Data Retrieval for Large Language Models' (Lin et al., 2024)
Paper: https://arxiv.org/abs/2405.11724
GitHub: https://github.com/huawei-lin/RapidIn
"""
...
class RapidGrad():
    def __init__(self, config, map_location, seed=42):
        ...
        self.M = 1
        self.shuffle_lambda = 20 * 2 #self.shuffle_lambda = config.influence.RapidGrad.shuffle_lambda*2
        self.perm_mat_list = []
        self.perm_dim_list = []
        self.grads_path = "./grads_path/" #self.config = config
    ...
    def __call__(self, vec, K):
        if self.D != len(vec):
            self.is_init = False
            self.D, self.random_mat, self.perm_mat_list, self.perm_dim_list = None, None, [], []
    ...
    def init(self, D):
        ...
        self.file_name = os.path.join(
            self.grads_path, #self.config.influence.grads_path,
