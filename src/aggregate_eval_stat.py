...
def get_forget_quality(unlearn_result, retain_result):
    ...
    if len(unlearn_forget_result.get('average_perturb_loss', {})) == 0:
        return {'Forget Quality': 0, 'KS Test PVal Forget': 0, 'KS Test Forget': 0}
...
def get_model_utility(eval_result_dict):
    ...
    for k, v in eval_result_dict.items():
        # getting Probability
        if len(eval_result_dict[k].get('average_perturb_loss', {})) > 0:
            if 'eval_log' in k:
            ...
            output_result[f'Prob. {eval_task_dict[k]}'] = 0
        ...
        # getting Truth Ratio
        if len(eval_result_dict[k].get('avg_paraphrased_loss', {})) > 0:
            ...
            output_result[f'Truth Ratio {eval_task_dict[k]}'] = 0
...
def main(cfg):
    ...
    print(f"Saved aggregated stats to {cfg.save_file}")
