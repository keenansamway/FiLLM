"""
Implementation of the SophiaG optimizer for PyTorch.
From: https://github.com/kyegomez/Sophia
"""
import math
import torch
...
class SophiaLocal(Optimizer):
    ...
    def __init__(self, params, lr=1e-4, betas=(0.965, 0.99), rho = 0.04,
        ...
        super(SophiaLocal, self).__init__(params[0], defaults)
    ...
    def update_hessian(self):
        ...
        for group in self.param_groups:
            ...
            local_params = [f"{n}.mlp" for n in range(0, 24)]
            for _, p in group['params']:
                if p.grad is None:
                    continue
                if all([p not in _ for p in local_params]):
                    continue
                state = self.state[p]
                ...
                # h_hat = p.grad.mul(p.grad)
                # state['hessian'].mul_(beta2).add_(h_hat, alpha=1 - beta2)
    ...
    def _single_tensor_sophiag(self, params: List[Tensor],
        ...
        for i, param in enumerate(params):
            ...
            # !test whether this is needed, might be updating the EMA twice
            # => quickly tested and after one epoch doesn't appear to make a huge different whether in or out
