...
Methods tested
    - LOCAL_LAYERS=("all" "mlp" "self-attn" "layernorm" "all random" "mlp random")

'
export CUDA_VISIBLE_DEVICES=0

experiment_name="layer_comparison_k0.1"
...
local_k=0.1
LOCAL_IN_SCOPE=("mlp\,attn\,norm\,embed" "mlp" "attn" "norm" "mlp\,attn\,norm\,embed" "mlp")
local_out_scope="bias"
LOCAL_SHUFFLE=("False" "False" "False" "False" "True" "True")
compression_power=16

for idx in "${!LOCAL_IN_SCOPE[@]}"
do
    local_in_scope=${LOCAL_IN_SCOPE[$idx]}
    local_shuffle=${LOCAL_SHUFFLE[$idx]}
