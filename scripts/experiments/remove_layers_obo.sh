...
experiment_name="remove_layers_obo"
...
if [ $1 == "tofu" ]; then
    ...
    forget_epochs=12
    forget_lr="5e-06"
    unlearn_split="forget10" # ("forget10" "forget05" "forget01")
    retain_split="retain90" # ("retain90" "retain95" "retain99")
    forget_checkpoint="checkpoint-30" # tofu: floor(40 * epochs / batch_size * gradient_accumulation_steps)
    save_steps=0
...
local_k=1
LOCAL_IN_SCOPE=("mlp\,attn\,norm\,embed" "mlp\,attn\,norm" "mlp\,attn" "mlp" "attn" "attn")
LOCAL_OUT_SCOPE=("none" "none" "none" "none" "none" "bias")
local_shuffle="False"
compression_power=16

for idx in "${!LOCAL_IN_SCOPE[@]}"
do
    local_in_scope=${LOCAL_IN_SCOPE[$idx]}
