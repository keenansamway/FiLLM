...
if [ $1 == "tofu" ]; then
    ...
    forget_epochs=8
    forget_lr="5e-05"
    unlearn_split="forget05" # ("forget10" "forget05" "forget01")
    retain_split="retain95" # ("retain90" "retain95" "retain99")
    FORGET_CHECKPOINT=({12..96..12}) # tofu: floor({40,200,400} * epochs / batch_size * gradient_accumulation_steps)
    save_steps=12 # 8 checkpoints
